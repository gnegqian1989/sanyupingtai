...
class MyTest(unittest.TestCase):
    ...
    def tearDown(self):
        print('end test')
        pass
class test_queryData(MyTest):
    '''测试接口：1.	1.	选择当前时间和当前时间7天后时间，列表数据'''

    def test_queryData(self):
        '''测试用例1：1.	1.	选择当前时间和当前时间7天后时间，列表数据'''
        ...
        self.url =(config.TestPlanUrl) +"/affairs/analysis/forcast/queryData.do?"\
                                        "dtDate=2018-11-23&" \
                                        "strCode=01030301&" \
                                        "timeDate=night&" \
